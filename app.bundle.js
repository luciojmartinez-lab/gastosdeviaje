...
async function imageGpsForFile(file, options = {}) {
  ...
  if (point === undefined) {
    point = null;
    try {
      imageLocationModulePromise ||= import('./image-location.js?v=700v117');
...
async function readExpenseTicket(prefix) {
  ...
  try {
    ...
    ticketOcrModulePromise ||= import('./ticket-ocr.js?v=700v117');
...
async function createDailyMapBlogImage(records, day) {
  ...
  records.forEach((record, index) => {
    ...
    const labelY = Math.max(headerHeight + 20, Math.min(headerHeight + mapHeight - 10, y - 12 + (index % 3) * 17));
    context.fillStyle = '#ffffffdd';
    context.fillRect(labelX - 4, labelY - 15, textWidth + 8, 20);
