...
function addCurrencyTotal(totals, currency, amount) {
  const code = String(currency || 'EUR').toUpperCase();
  totals[code] = numberValue(totals[code]) + numberValue(amount);
  return totals;
}

function mergeCurrencyTotals(target, source) {
  Object.entries(source || {}).forEach(([currency, amount]) => addCurrencyTotal(target, currency, amount));
  return target;
}

function expenseCurrencyTotals(expenses) {
  return (Array.isArray(expenses) ? expenses : []).reduce(
    (totals, expense) => addCurrencyTotal(totals, expense.moneda, expense.importe),
    {}
  );
}

function formatForeignCurrencyTotals(totals, empty = '—') {
  const lines = Object.entries(totals || {})
    .filter(([currency, amount]) => currency !== 'EUR' && Math.abs(numberValue(amount)) > 0.000001)
    .sort(([left], [right]) => left.localeCompare(right))
    .map(([currency, amount]) => `<span class="original-currency-total">${fmtCurrency(amount, currency)}</span>`);
...
function timelineRecordMaxSpreadMeters(record) {
  const points = timelineRecordPointCandidates(record);
  let spread = 0;
  for (let first = 0; first < points.length; first += 1) {
    for (let second = first + 1; second < points.length; second += 1) {
      spread = Math.max(spread, lodgingDistanceMeters(points[first], points[second]));
    }
  }
  return spread;
}
...
function timelineImportAdjustmentPlan(records, changedDates, hasAdjustment) {
  const changed = records.filter(record => changedDates.has(record.fecha));
  return {
    pending: changed.filter(record => !hasAdjustment(record)),
    preserved: changed.filter(record => hasAdjustment(record))
  };
}
...
function renderTimelineDialog() {
  ...
  const mapsAvailable = timelineRecordHasAdjustableMaps(dayRecord, exactRecords);
  if (routeAdjust) {
    routeAdjust.disabled = !dayRecord || !mapsAvailable || stationaryDay || timelineBulkRoutingActive;
  ...
  const pendingDays = records.filter(record =>
    timelineRecordHasAdjustableMaps(record, exactRecords)
    ...
    routeHelp.textContent = !tripMapState.day
      ...
      : !dayRecord
        ...
        : dayGpx
          ? mapsAvailable
            ? cached
              ? `${blogDayDateLabel(dayRecord.fecha)} conserva el GPX directo y tiene ajustados los tramos complementarios de Maps con ${cached.provider || 'Valhalla / OpenStreetMap'}.`
              : `${blogDayDateLabel(dayRecord.fecha)} conserva el GPX directo. Ajustar el día modificará únicamente los intervalos complementarios de Maps.`
            : `${blogDayDateLabel(dayRecord.fecha)} queda cubierto por el GPX y no contiene tramos adicionales de Maps que ajustar.`
        : stationaryDay
...
async function adjustSelectedTimelineDay(options = {}) {
  ...
  const gpxPaths = timelineRecordGpxPaths(record);
  const originalPaths = gpxPaths.length
    ? timelineMapsPathsOutsideGpx(record, exactRecords)
    : timelineMapPaths([record], { exactRecords });
  const routingRecords = gpxPaths.length
    ? timelineRecordsOutsideGpxIntervals(exactRecords, gpxPaths)
    : exactRecords;
  const sourcePaths = timelineMapPathsWithDailyRecords(originalPaths, routingRecords);
  if (!sourcePaths.length) throw new Error(gpxPaths.length
    ? 'El GPX ya cubre el recorrido y no hay intervalos complementarios de Maps que ajustar.'
    : 'Ese día no tiene suficientes puntos para ajustar el recorrido.');
...
async function adjustEntireTimelineTrip() {
  ...
  const exactRecords = dailyMapRecordsForScope(new Set([Number(trip.id)]), 0);
  const records = timelineRecordsForTrip(trip.id)
    .filter(record => timelineRecordHasAdjustableMaps(record, exactRecords))
...
function parseTimelineFileInWorker(file, trip) {
  ...
  return new Promise((resolve, reject) => {
    const worker = new Worker('./timeline-import-worker.js?v=700v314');
...
async function importTimelineFile(file) {
  ...
  try {
    ...
    if (autoAdjust && adjustmentPlan.pending.length) {
      timelineBulkRoutingActive = true;
      timelineBulkRoutingCancelled = false;
      renderTimelineDialog();
      for (let index = 0; index < adjustmentPlan.pending.length; index += 1) {
        const record = adjustmentPlan.pending[index];
        try {
    ...
    renderTripMap();
    renderTimelineDialog();
    const preservedText = adjustmentPlan.preserved.length
      ? ` Se ${adjustmentPlan.preserved.length === 1 ? 'conservó el ajuste anterior de 1 día' : `conservaron los ajustes anteriores de ${adjustmentPlan.preserved.length} días`}; usa Recalcular el día solo si quieres sustituirlo.`
      : '';
    const adjustmentText = autoAdjust && adjustmentPlan.pending.length
      ? ` Ajuste automático: ${adjustedCount} ${adjustedCount === 1 ? 'día nuevo ajustado' : 'días nuevos ajustados'}${adjustmentFailures ? ` y ${adjustmentFailures} sin ajustar` : ''}.`
      : '';
    setTimelineMessage(`Importación terminada: ${result.summary.dayCount} días y ${result.summary.pointCount} puntos del recorrido.${preservedText}${adjustmentText}`);
  } finally {
...
async function deleteTimelineGpxForSelectedDay() {
  ...
  if (timelineRecordHasMaps(record)) {
    ...
    await putRecord('timelineDays', { ...withoutGpx, adjustedTimelineRoutes: {} });
  } else {
    await deleteRecord('timelineDays', record.id);
  }
  await loadAll();
...
async function readImageMetadataForFile(file) {
  ...
  const metadataPromise = (async () => {
    ...
    if ((!imageGpsCache.has(file) || !imageDateTimeCache.has(file)) && canContainExif) {
      try {
        imageLocationModulePromise ||= import('./image-location.js?v=700v314');
...
function suggestTicketCategory(text, merchant, foodEvidence = null) {
  ...
  const categoryHaystack = haystack
    // Lens can corrupt "relacionado" while leaving both ends of this payment
    // description intact. It identifies an IC-card payment, not a transport expense.
    .replace(/\bdinero\s+electronico\b[^\n]{0,120}\btransporte\b/g, ' ')
    .replace(
  ...
  const rules = [
    ...
    { words: ['gasolinera', 'gasolina', 'gasoleo', 'combustible', 'repsol', 'cepsa', 'bp '], categories: ['Transporte'], subcategories: ['Combustible', 'Gasolina'] },
...
async function recognizeExpenseTicketSource(prefix, source, options = {}) {
  ...
  try {
    ...
    ticketOcrModulePromise ||= import('./ticket-ocr.js?v=700v314');
...
function renderGastosTabla() {
  ...
  groupKeys.forEach(key => {
    ...
    byGroup[key].sort(compareExpensesChronologically).forEach(g => {
      ...
      subtotalEur += eur;
      totalEur += eur;
      addCurrencyTotal(subtotalCurrencies, g.moneda, g.importe);
      addCurrencyTotal(totalCurrencies, g.moneda, g.importe);
      const tr = document.createElement('tr');
    ...
    subtotal.innerHTML = `<td colspan="6" style="text-align:right"><i>Subtotal</i></td><td class="original-currency-totals">${formatForeignCurrencyTotals(subtotalCurrencies)}</td><td>${fmtCurrency(subtotalEur, 'EUR')}</td><td colspan="2"></td>`;
    tbody.appendChild(subtotal);
  });
  const totalOriginal = $('#tg-original');
  if (totalOriginal) totalOriginal.innerHTML = formatForeignCurrencyTotals(totalCurrencies);
...
function timelineGpxRouteSignature(route) {
  if (!route) return '';
  const segmentLengths = (Array.isArray(route.segments) ? route.segments : [])
    .map(segment => Array.isArray(segment) ? segment.length : 0);
  return JSON.stringify([
    String(route.sourceName || ''),
    Number(route.sourceSize || 0),
    Number(route.sourceLastModified || 0),
    Number(route.pointCount || 0),
    String(route.startTime || ''),
    String(route.endTime || ''),
    segmentLengths
  ]);
}

function timelineGpxTimeIntervals(gpxPaths) {
  return gpxPaths
    .map(path => {
...
function timelineTimeCoveredByGpx(value, intervals) {
  const time = Date.parse(value || '');
  return Number.isFinite(time) && intervals.some(interval => time >= interval.start && time <= interval.end);
}

function timelinePathsOutsideGpxIntervals(paths, gpxPaths) {
  const intervals = timelineGpxTimeIntervals(gpxPaths);
  ...
  paths.forEach(path => {
    ...
    path.forEach(point => {
      const covered = timelineTimeCoveredByGpx(point && point.time, intervals);
      if (covered) flush();
...
function timelineRecordsOutsideGpxIntervals(records, gpxPaths) {
  const intervals = timelineGpxTimeIntervals(gpxPaths);
  if (!intervals.length) return records;
  return (records || []).filter(record => {
    const value = record && record.time
      || `${record && record.fecha || ''}T${record && record.hora || ''}`;
    return !timelineTimeCoveredByGpx(value, intervals);
  });
}

function timelineMapsPathsOutsideGpx(record, exactRecords = []) {
  const gpxPaths = timelineRecordGpxPaths(record);
  if (!gpxPaths.length || !timelineRecordHasMaps(record)) return [];
  const mapsRecord = { ...record, gpxRoute: null };
  const mapsPaths = timelineMapPaths([mapsRecord], { exactRecords, adjusted: false });
  return timelinePathsOutsideGpxIntervals(mapsPaths, gpxPaths);
}

function timelineRecordHasAdjustableMaps(record, exactRecords = []) {
  if (!record || !timelineRecordHasMaps(record)) return false;
  const gpxPaths = timelineRecordGpxPaths(record);
  return !gpxPaths.length || timelineMapsPathsOutsideGpx(record, exactRecords).length > 0;
}

function timelineCachedMapPaths(record, options = {}) {
...
function timelineMapPaths(records) {
  ...
  if (records.some(record => gpxPathsForRecord(record).length)) {
    return records.flatMap(record => {
      ...
      const cachedMapsPaths = options.adjusted === true ? timelineCachedMapPaths(record, options) : [];
      const mapsPaths = cachedMapsPaths.length
        ? cachedMapsPaths
        : timelineMapsPathsOutsideGpx(record, options.exactRecords || []);
      return [...mapsPaths, ...gpxPaths];
    });
  }
  if (options.adjusted === true) {
    return records.flatMap(record => {
      if (timelineRecordIsStationaryNoise(record, options.exactRecords)) return [];
      const cachedPaths = timelineCachedMapPaths(record, options);
      return cachedPaths.length ? cachedPaths : timelineMapPaths([record], { exactRecords: options.exactRecords });
    });
  }
  const paths = [];
  records.forEach(record => {
...
function renderResumen() {
  ...
  const totalsByCurrency = expenseCurrencyTotals(gastos);
  ...
  gastos.forEach(g => {
    ...
    const item = byCategory[key] || { total: 0, currencies: {} };
    item.total += toEur(g.importe, g.moneda);
    addCurrencyTotal(item.currencies, g.moneda, g.importe);
    byCategory[key] = item;
  });
  ...
  const categoryTotals = categoryRows
    .reduce((items, row) => {
      ...
      if (found) {
        ...
        mergeCurrencyTotals(found.currencies, row.currencies);
      } else items.push({ cat: row.cat, total: row.total, currencies: { ...row.currencies } });
      return items;
    }, [])
    .sort((a, b) => b.total - a.total);
  const breakdownMode = $('#r-desglose') ? $('#r-desglose').value : 'categorias';
  const breakdownHead = $('#tabla-cat thead tr');
  if (breakdownHead) breakdownHead.innerHTML = '<th>Categoría</th><th>Subcategoría</th><th>Moneda original</th><th>Total EUR</th><th>% gasto</th>';
  if (breakdownMode === 'categorias') {
    ...
    categoryTotals.forEach(catRow => {
      categoryRows
        ...
        .sort((a, b) => b.total - a.total)
        .forEach(row => {
          groupedRows.push(breakdownRow(row.cat, row.sub, row.total, row.currencies));
          pieRows.push(row);
        });
          groupedRows.push(breakdownRow(catRow.cat, 'Subtotal categoría', catRow.total, catRow.currencies, 'subtotal-row'));
    });
  ...
  if (breakdownMode === 'ciudades') {
    ...
    gastos.forEach(g => {
      ...
      const item = cityTotals[key] || { total: 0, currencies: {} };
      item.total += toEur(g.importe, g.moneda);
      addCurrencyTotal(item.currencies, g.moneda, g.importe);
      cityTotals[key] = item;
    });
    const cityRows = Object.entries(cityTotals)
      ...
      .sort((a, b) => b.total - a.total);
    if (breakdownHead) breakdownHead.innerHTML = '<th>Ciudad</th><th>País</th><th>Moneda original</th><th>Total EUR</th><th>% gasto</th>';
  ...
  const accountSpentCurrencies = accountRows.reduce((totals, row) => addCurrencyTotal(totals, row.moneda, row.total), {});
  const accountBalanceCurrencies = accountRows.reduce((totals, row) => addCurrencyTotal(totals, row.moneda, row.saldo), {});
  ...
  accountHtml.push(`<tr class="subtotal-row${isOverTripBudget ? ' over-budget-row' : ''}"><td data-label="Cuenta"><span class="account-label-full">Total cuentas</span><span class="account-label-mobile">Total</span></td><td data-label="Moneda">EUR + divisas</td><td data-label="Gastado">${fmtCurrency(totalEur, 'EUR')}<span class="original-currency-totals">${formatForeignCurrencyTotals(accountSpentCurrencies, '')}</span></td><td data-label="% gastos">${totalEur ? '100%' : '0%'}</td><td data-label="Saldo">${fmtCurrency(accountBalanceEur, 'EUR')}<span class="original-currency-totals">${formatForeignCurrencyTotals(accountBalanceCurrencies, '')}</span></td><td data-label="% saldo">${accountBalanceEur ? '100%' : '0%'}</td><td data-label="EUR"></td></tr>`);
...
async function blogShareCanvasPdfBlob(canvas) {
  ...
  blogSharePdfModulePromise ||= import('./share-pdf.js?v=700v314');
...
async function routeSharedExpenseImages(prefix, files, kind, action = 'replace', options = {}) {
  ...
  if (options.fromLens && options.spanishOnly) {
    ...
    await readLensTicketTranslation(prefix, lensRecord, first, options);
    return;
  }
  if (action === 'translation' || action === 'secondary') {
    ...
    renderExpenseTicketTranslation(prefix);
    if (action === 'translation') await readLensTicketTranslation(prefix, companion, first, options);
...
function bindEvents() {
  ...
  $('#add-gasto-close').onclick = cancelAddGasto;
  $('#add-gasto-cancel').onclick = cancelAddGasto;
  $('#add-gasto-dialog').oncancel = cancelAddGasto;
