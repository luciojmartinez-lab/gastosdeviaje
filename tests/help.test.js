...
test('la ayuda identifica fecha y versión y contiene las tres partes principales', async () => {
  ...
  assert.match(html, /Última actualización:<\/strong> 11 de agosto de 2026/);
  assert.match(html, /Versión documentada:<\/strong> 700v247/);
...
test('todos los modales tienen ayuda contextual con un destino documentado', async () => {
  ...
  assert.equal(allDialogIds.length, 22);
  assert.equal(dialogIds.length, 20);
