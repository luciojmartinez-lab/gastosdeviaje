...
test('el service worker se activa sin descargar archivos y prepara la cache despues', () => {
  ...
  assert.match(sw, /\.\/ticket-image-worker\.js\?v=700v283/);
  assert.match(sw, /\.\/ticket-image-processing\.js\?v=700v283/);
  assert.match(sw, /\.\/timeline-import\.js\?v=700v283/);
  assert.match(sw, /\.\/timeline-import-worker\.js\?v=700v283/);
  assert.match(sw, /\.\/timeline-routing\.js\?v=700v283/);
...
test('una versión nueva provoca una sola recarga después de activar su service worker', () => {
  ...
  assert.match(sw, /const APP_VERSION = '700v283'/);
