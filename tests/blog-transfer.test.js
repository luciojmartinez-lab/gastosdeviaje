...
test('añadir una imagen al editar conserva la fecha y evita dos ventanas modales apiladas', () => {
  ...
  assert.match(styles, /#edit-gasto-dialog \{[\s\S]*?height: calc\(100svh - 18px\);[\s\S]*?max-height: calc\(100svh - 18px\);[\s\S]*?box-sizing: border-box;[\s\S]*?margin: 9px auto auto;[\s\S]*?overflow-anchor: none;[\s\S]*?overscroll-behavior: contain;[\s\S]*?scrollbar-gutter: stable/);
