...
(function(){
  ...
  function renderResumenOverride(){
    ...
    try{
      ...
      let porCBase = cuentasResumen.map(c=>{ const dato=resumenPorCuentaPorId.get(String(c.id)); if(dato) return dato; const pres=+c.presupuesto>0? +c.presupuesto:0; return {id:c.id,label:c.nombre,moneda:c.moneda,total:0,presupuesto:pres,pct:0}; }).filter(Boolean);
      if(!porCBase.length) porCBase = resumenPorCuenta.slice();
      const porCTabla = (cta
